...
class page {
    ...
    private function myde_prev() {
        if ($this->myde_page != 1) {
            return "<li class=paginate_button ><a href='" . $this->page_replace($this->myde_page - 1) . "' title='上一頁'> < </a></li>";
        } 
    }
    ...
    private function myde_next() {
        if ($this->myde_page != $this->myde_page_count) {
            return "<li class=paginate_button ><a href='" . $this->page_replace($this->myde_page + 1) . "' title='下一頁'> > </a></li>";
        } else {
            return"<li class=paginate_button ><a> > </a></li>";
        }
    }
    ...
    public function myde_write($id = 'page') {
        ...
        for ($i = $this->myde_i; $i <= $this->myde_en; $i++) {
            if ($i == $this->myde_page) {
                $str.="<li class='paginate_button active'><a href='" . $this->page_replace($i) . "' title='第" . $i . "頁' >$i</a></li>";
            } else {
                $str.="<li class=paginate_button ><a href='" . $this->page_replace($i) . "' title='第" . $i . "頁'>$i</a></li>";
            }
        }
        if ($this->myde_en < $this->myde_page_count) {
            //$str.="<p class='pageEllipsis'>...</p>";
