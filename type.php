<?php
	...
	$typ = $get["typ"];
	if($typ=="" || $Type[$typ]==""){header("location: index.php"); exit;}
	$type = $Type[$typ];
?>
<!doctype html>
    ...
    <div class="container" style="padding:0px; min-height: 400px;">
        <div id="newsdiv">
            ...
            <div class="selectdiv" align="right">
                <select name="id" id="id" onChange="location.href='detail.php?typ=<?=$typ?>&id='+this.value;">
                    <option value="">請選擇報表議題</option>
                <?php
                    $rs=SelectSqlDB("select id,title from chart where type=N'".$type."' and isenable=1 order by sort,id desc");
        ...
        <div id="iframe_c">
        <?php
            $rs=SelectSqlDB("select * from chart where type like (N'%".$type."%') and isenable=1 order by sort,id desc");
